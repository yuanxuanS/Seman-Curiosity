...
def _log_classification_stats(pred_logits, gt_classes, prefix="fast_rcnn"):
    ...
    if num_instances == 0:
        return
    pred_classes = pred_logits.argmax(dim=1)    # 预测的类别的索引
    bg_class_ind = pred_logits.shape[1] - 1     # 背景的索引

    fg_inds = (gt_classes >= 0) & (gt_classes < bg_class_ind)   # 筛选前景类别
    num_fg = fg_inds.nonzero().numel()
    fg_gt_classes = gt_classes[fg_inds] # 前景样本的真实类别
    fg_pred_classes = pred_classes[fg_inds] # 前景样本的预测类别

    num_false_negative = (fg_pred_classes == bg_class_ind).nonzero().numel()    # FN: 前景被预测成背景； FP：预测错物体
