...
class FixedVoxelGrid(Structure):
    ...
    def __init__(self,
                 *,
                 ...
                 n_x=1, n_y=1, n_z=1,
                 size_x=None, size_y=None, size_z=None,
                 regular_bounding_box=False, lower_bound, upper_bound):
        """Grid of voxels with support for different build methods.
    ...
    def compute(self):
        """ABC API."""
        xyzmin = self.lower_bound #self._points.min(0)
        xyzmax = self.upper_bound #self._points.max(0)
        xyz_range = self._points.ptp(0)
