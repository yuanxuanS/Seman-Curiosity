...
json_dir = "./third_parties/detectron2/datasets/embodied_sequencev2_wotrajR_bg4_r3_his/"
...
with open("./asample_straight_sampled_sequencev2_wotrajR_his_bg4_r3_rewrite.pkl", "rb") as f:
...
for env, episode, glbstep, step in zip(inputs[0], inputs[1], inputs[2], inputs[3]):
    ...
    if not [env, episode, glbstep, step] in glb_frames_indices:
        continue

    if save_pos:
        type.append("position")
    sample_data = sampler.get_sample_multimodality(
        env, episode, step, type, glbstep)
    
    name = f"epi{episode}_env{env}_glb_{glbstep}_step{step}.png"
            ...
            coco_dict['annotations'].append(
                {
                    ...
                    "iscrowd": 0,
                    # other key
                    "env_episode_glb_step": [int(env), int(episode), int(glbstep), int(step)],
                    # "object_id": env,
