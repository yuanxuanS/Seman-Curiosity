...
def construct_envs(args):
    ...
    if len(scenes) > 0:
        ...
        scene_split_sizes = [int(np.floor(len(scenes) / args.num_processes))     # 每个线程分配的环境数；将所有环境平均分配到每个线程
                             for _ in range(args.num_processes)]
        for i in range(len(scenes) % args.num_processes):   # 余出来的环境加在前面线程
    ...
    for i in range(args.num_processes):
        ...
        if len(scenes) > 0:
            config_env.DATASET.CONTENT_SCENES = scenes[ # 按照分配的数量分配该线程的场景
        ...
        config_env.SIMULATOR.HABITAT_SIM_V0.GPU_DEVICE_ID = gpu_id      # habitat的环境所在gpu？
