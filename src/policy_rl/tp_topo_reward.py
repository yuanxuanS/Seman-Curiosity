    ...
    def update(self, env_indices, features, positions, has_targets):
        ...
        for i, env_idx in enumerate(env_indices):
            ...
            if np.min(dists) >= self.dist_threshold:
                self.env_nodes[env_idx].append([curr_p, curr_feat])
                rewards[i] = 1.0
                self.dist_threshold =min(6, self.w * self.dist_threshold + (1-self.w) * np.min(dists) )
...
class VectorizedTopologyManagerFeatureDistance:
    def __init__(self, num_envs, dist_threshold=0.5, dist_feat_threshold=6, check_target=False):
        self.num_envs = num_envs
        self.dist_threshold = dist_threshold
        self.dist_feat_threshold = dist_feat_threshold
        self.w = 0.8
        # 为每个环境维护一个独立的节点列表
        self.env_nodes = [[] for _ in range(num_envs)]
        self.check_target = check_target
    ...
    def update(self, env_indices, features, positions, has_targets):
        ...
        for i, env_idx in enumerate(env_indices):
            ...
            # 3. 全局距离约束 (计算当前点到该环境所有已存节点的距离)
            node_dists = np.array([nf[0] for nf in nodes])
            dists = np.linalg.norm(np.array(node_dists) - curr_p, axis=1)
            if np.min(dists) >= self.dist_threshold:        # 满足空间距离阈值
                node_features = np.array([nf[1] for nf in nodes])
                dists_feat = np.linalg.norm(node_features - curr_feat, axis=1)
                if np.min(dists_feat) >= self.dist_feat_threshold:  # 满足特征距离阈值
                    # print(f" min feature dists, {np.min(dists_feat)}")
                    self.env_nodes[env_idx].append([curr_p, curr_feat])
                    rewards[i] = 1.0 * np.min(dists_feat) / self.dist_feat_threshold    # 奖励为特征差距 
                    # thres update
                    # self.dist_feat_threshold =self.w * self.dist_feat_threshold + (1-self.w) * np.min(dists_feat) 
                    self.rs_dis_feat_thres1 = self.dist_feat_threshold * 0.2
                    self.rs_dis_feat_thres2 = self.dist_feat_threshold * 0.4
                else:
                    # print(f" min dists, {np.min(dists)}")
                    rewards[i] = 0.5
            else:
