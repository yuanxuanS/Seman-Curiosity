...
class VQF_Policy(NNBase):
    '''
    state: 
        - 4 full maps + 4 local maps + 1 target semantic maps
        - no orient: 默认约束为朝向物体
        - no goal 
    ...
    def __init__(self, input_shape, recurrent=False, hidden_size=512,
        ...
        super(VQF_Policy, self).__init__(
...
class RL_VQF_Policy(nn.Module):

    def __init__(self, obs_shape, action_space, model_type=0,
                 base_kwargs=None):

        super(RL_VQF_Policy, self).__init__()
