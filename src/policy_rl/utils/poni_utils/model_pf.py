...
from .default import get_cfg, Config
from .dataset import SemanticMapDataset as PFDataset
import src.policy_rl.utils.poni_utils.geometry as pgeo
...
class Potential_Function_Semantic_Policy(nn.Module):
    def __init__(self, pf_model_path):
        ...
        loaded_cfg = Config()
        loaded_cfg.defrost()
        loaded_cfg.merge_from_file(pf_model_path+"_config.yaml")
        loaded_cfg.freeze()
        
        pf_model_cfg = get_cfg()
        pf_model_cfg.merge_from_other_cfg(loaded_cfg)
        self.pf_model = PFModel(pf_model_cfg)
        
        loaded_state_dict = torch.load(pf_model_path+"_statedict.pth", map_location="cpu")
...
class RL_Policy(nn.Module):
    ...
    def act(
        ...
        if False:       #self.needs_egocentric_transform:
            # Input conventions:
