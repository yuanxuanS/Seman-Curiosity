...
try:
    ...
    if habitat_version == "0.1.5": # or "0.1.7":
        print("Using habitat 0.1.X; assuming SemExp code is being used")

        class PointNavResNetTensorOutputPolicy(PointNavResNetPolicyV2):
        ...
        class PointNavResNetTensorOutputPolicy(PointNavResNetPolicyV2):  # type: ignore
            def act(self, *args: Any, **kwargs: Any) -> Tuple[Tensor, Tensor]:
    ...
    from src.policy_rl.utils.non_habitat_policy.nh_pointnav_policy import (
...
def load_pointnav_policy(file_path: str) -> PointNavResNetTensorOutputPolicy:
    ...
    if HABITAT_BASELINES_AVAILABLE:
        ...
        if habitat_version == "0.1.5": # or "0.1.7":
            ...
            ckpt_dict = torch.load(file_path+"_statedict.pth", map_location="cpu")
            from omegaconf import OmegaConf
            ckpt_config = OmegaConf.load(file_path+"_config.yaml")
            # breakpoint()
            pointnav_policy = PointNavResNetTensorOutputPolicy.from_config(ckpt_config, obs_space, action_space)
