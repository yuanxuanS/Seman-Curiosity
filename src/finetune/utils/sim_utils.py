...
class FirstPersonImageExtractor(ImageExtractor):
    ...
    def __init__(
        ...
        assert set(self.SIM_TO_COCO_MAPPING.values()) == set(constants.coco_categories_mapping.keys())
        self.mode = "full"
        self._objects = self.sim.semantic_scene.objects
        self.mapping = self.instance_id_to_name

        self.filter_occluded_instances = False      # 很多被遮挡, 难以选到不遮挡的
    ...
    def get_bbs(self, *args, **kwargs):
        ...
        for id_object in objects_id:
            ...
            bounding_box = cv2.boundingRect((mask_cleaned == id_object).astype('uint8'))
            ...
            if habitat_id in self.SIM_TO_COCO_MAPPING:
                ...
                pred_mask[mask_cleaned == id_object] = True
                coco_id = self.SIM_TO_COCO_MAPPING[habitat_id]

                if self.filter_occluded_instances:      # TODO: 在干什么?
