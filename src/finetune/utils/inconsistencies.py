...
def solve_inconsistency3(
    ...
    for vox_id in vox_ids:
        ...
        # record compured res
        key = tuple(sorted(ids))    # ids是unique一定不会重复
        if key not in computed_dict:
            resolved_class, _ = solve_function(logits)
            computed_dict[key]=resolved_class
        else:
            resolved_class = computed_dict[key]
        results.append((vox_id, (resolved_class, logits)))
