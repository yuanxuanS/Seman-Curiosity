...
class SemanticPointCloud:
    ...
    def preprocess(self, lower_bound=None, upper_bound=None):
        ...
        voxelgrid_id = _pcd.add_structure(       # spend most time
            "fixedvoxelgrid",
            n_x=d_x,
            n_y=d_y,
            n_z=d_z,
    ...
    def get_topdown_semantic(self, lower_bound, upper_bound, map_scale=0.025):
        ...
        if len(self) and self.points is not None:
            ...
            explore_mat = cv2.morphologyEx(explore_mat, cv2.MORPH_CLOSE, kernel)    # 形态学闭运算
    ...
    def _depth_raytracing(self, depth, pose):
        ...
        projected_points = _project_batch_masks(depth, pose).numpy()    # 将depth投影
        ...
        object_ids = self.object_ids[founds[1][keep]]       # 同一个物体的id
