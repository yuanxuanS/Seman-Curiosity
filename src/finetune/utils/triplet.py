...
def online_mine_hard(labels, embeddings, margin=0.2, squared=False, device='cpu'):
    ...
    mask_anchor_positive = _get_anchor_positive_triplet_mask(labels, device)    # 提取相同label的对， batch, batch
    ...
    # shape (batch_size, 1)
    hardest_positive_dist = torch.max(anchor_positive_dist, 1, keepdim=True)[0] # 正样本对之间最大距离，是最难正样本对
