...
def main():
    ...
    l_process_rewards = np.zeros((num_scenes))
    ...
    l_observation_space = envs.get_obs_space()  # TODO: VectorEnv's func
    l_action_space = envs.get_action_space()
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        if done[0]:     # maps are new obs, sum of map will be small, and get negative reward
            l_reward = args.reward_coeff*last_reward
        else:
            l_reward = args.reward_coeff* maps.sum_of_semantic_map()

        l_process_rewards += l_reward.cpu().numpy()
        ...
        print(f"step: {step} , local step: {l_step}, reward={reward}")
        logging.info(f"step: {step} , local step: {l_step}, reward={reward}")
        ...
        # pred instance, get semantic masks and step env
        obs, _, done, infos = envs.step_and_preprocess(l_action, vis_inputs)    # if done ,obs are ones after reset
        ...
        if (step * num_scenes) % args.save_interval < \
                num_scenes:
            if len(l_episode_rewards) >= 1000 and \
                    (np.mean(l_episode_rewards) >= best_g_reward) \
                    and not args.eval:
                torch.save(l_policy.state_dict(),
                           os.path.join(log_dir, "model_best.pth"))
                best_g_reward = np.mean(l_episode_rewards)
