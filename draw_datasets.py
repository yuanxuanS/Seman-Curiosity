    ...
    for batch_idx, batch in enumerate(test_loader):
        for idx, x in enumerate(batch):
            ...
            if with_mask:
                ...
                for n in range(iter):
                    if y.gt_classes[n] not in list(coco_categories_mapping.values()):
                        continue
                    masks = y.gt_masks.tensor[n].cpu().numpy()[:,:,None]
                    image = x['image'].permute(1, 2, 0).cpu().numpy() * masks
                    ...
                    cv2.imwrite(save_pth + "/mask"+f"_{n}"+"_epi"+str(x['episode'])+"_env"+str(x['env']) + "_step"+str(x['step'])+".png", frame)
            else:
                visualizer = Visualizer(
                    deepcopy(x['image'].permute(1, 2, 0).cpu().numpy()),
                    metadata,
                    instance_mode=ColorMode.IMAGE,
                )
