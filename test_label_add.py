...
if __name__ == "__main__":
    ...
    class_names = ["refrigerator",]    #"couch" , "bed", ,"chair" , ， "toilet",
    visualize = False        # 可视化该物体的GT
    ...
    for class_name in class_names:
        ...
        for obj_dir in os.listdir(dir):
            ...
            for distance in distances:
                if distance in list(values[obj_dir].keys()):    # 默认包含该距离下所有样本
                    continue
                else:
                    print(f" no {distance} in {obj_dir}")
        ...
        # 存
        if predict_mode == "maskrcnn":
            with open(save_dir+ class_name+"_orig_ins.pkl", "wb") as f:
                pickle.dump(pred_instances, f)
            with open(save_dir+ class_name+"_maskrcnn.pkl", "wb") as f:
            ...
            with open(save_dir+ class_name+"_clip_add.pkl", "wb") as f:
                pickle.dump(values, f)
            ...
            for obj, obj_dict in values.items():
                for d in dis:
                    if visualize:
                        draw_array(obj, values[obj], class_name+"_clip10_norm11", norm=True)
                    value = values[obj][d]
