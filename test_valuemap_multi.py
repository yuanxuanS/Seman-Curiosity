...
if __name__ == "__main__":
    ...
    print(f" azimuth {azimuth}, confidence is {confidence}")

    obj_pose = torch.tensor([0, 0, -azimuth]).unsqueeze(0).to(device)
    
    rot_mat, trans_mat = get_grid(obj_pose, quality_field.size(),
                                    device)
    rotated_qf = F.grid_sample(quality_field.to(device), rot_mat, align_corners=True)
    ...
    point_cloud_t = du.get_point_cloud_from_z_t(
            torch.from_numpy(depth_obj), camera_matrix, 'cpu', scale=1)
    ...
    x1 =int(map_size_m // (XY_resolution * 2) - vision_range // 2 - dx_obj // XY_resolution)
    x2 = x1 + vision_range
    y1 = int(map_size_m // (XY_resolution * 2) + dy_obj // XY_resolution) # - vision_range // 2)     # field以地图中心为原点 ？
    y2 = y1 + vision_range
    ...
    data = agent_view.squeeze(0).squeeze(0).detach().cpu().numpy()  # 压缩为 [w, h]
