...
class panorama_policy(nn.Module):
     def __init__(self, config):
        super().__init__()
     ...
     def forward(self, 
                ob_img_feats, ob_ang_feats, 
                ):
         
         ob_embeds = self.img_embeddings(ob_img_feats, ob_ang_feats)
         attention_outputs = self.attention(ob_embeds,)[0]
...
if __name__ == '__main__':
    ...
    img_feats = process_features(args, device)
    print(img_feats.shape)
    
    angle_feat_size = 2
    env=5
    ang_feats = get_all_point_angle_feature(angle_feat_size, )
    ang_feats = (ang_feats.unsqueeze(0)).repeat(env, 1,1).to(device)
    
    policy = panorama_policy(model_config)
    policy = policy.to(device)
    
    # test
    # img_feats[0, ...] = torch.zeros_like(img_feats[0, ...]).to(device)
    # ang_feats[0, ...] = torch.zeros_like(ang_feats[0, ...]).to(device)
    act_prob = policy(img_feats, ang_feats)
    # img_embed = ImageEmbeddings(model_config)
    # embedding = img_embed(torch.from_numpy(img_feats), torch.from_numpy(ang_feats))
    print(act_prob)