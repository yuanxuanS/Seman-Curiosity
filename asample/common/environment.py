...
from habitat_extensions.utils import  heading_from_quaternion, navigator_video_frame, planner_video_frame
...
from asample.common.utils import  save_obs
from asample.common.semantic_prediction import SemanticPredMaskRCNN
from asample.constants import target_coco_categories_mapping
...
@baseline_registry.register_env(name="ActiveSampleEnv")
class ActiveSampleEnv(habitat.RLEnv):
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        ...
        self.video_frames = []
        self.plan_frames = []
        self.max_len = config.RL.episode_length
        
        self.curr_node_step = 0
        self.last_nstep = 0
        self.steps = 0
        self.episode_no = 0
        self.timestep = -1
        ...
        config.defrost()
        config.detector.sem_gpu_id = config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.GPU_DEVICE_ID
        config.freeze()
        det_args = self.config_to_parser(config.detector)
        self.sem_pred = SemanticPredMaskRCNN(det_args)
    
    def config_to_parser(self, config_obj ):
        parser = argparse.ArgumentParser(description="Auto-generated Parser from Config")
        
        # 遍历 dataclass 的所有字段
        for key, value in config_obj.items():
            
            # 处理布尔值的特殊逻辑（命令行通常用 --flag 或 --no-flag）
            if type(value) is bool:
                parser.add_argument(f"--{key}", action="store_true", default=value)
            else:
                parser.add_argument(f"--{key}", type=type(value), default=value)
        
        args = parser.parse_known_args([])        # 0: 已知参数 1: 未知参数list
        return args[0]
    ...
    def get_done(self, observations: Observations) -> bool:
        ...
        return self.curr_node_step == self.max_len
    ...
    def turn(self, ang, vis_info):    
        ''' angle: 0 ~ 360 degree '''
        act_l = 1   #HabitatSimActions.TURN_LEFT
        act_r = 2   # HabitatSimActions.TURN_RIGHT
        ...
        if 180 < ang_degree <= 360:
            ang_degree -= 360
        if ang_degree >=0:
            turns = [act_l] * int( ang_degree // uni_l)
        else:
            turns = [act_r] * (-ang_degree // uni_l)

        for turn in turns:
            observations = self.wrap_act(turn, vis_info, save=False)
        return observations
    ...
    def single_step_control(self, pos, tryout, vis_info):
        act_f = 0   #HabitatSimActions.MOVE_FORWARD
        uni_f = self._env.sim.agents[0].agent_config.action_space[1].actuation.amount
        agent_state = self._env.sim.get_agent_state()
        ...
        ksteps = min(int(dis // uni_f), 10)
        if not tryout:
            for _ in range(ksteps):
                self.wrap_act(act_f, vis_info, save=True)
        else:
            # 考虑到死锁等情况（paper的control部分）
            cnt = 0 
            for _ in range(ksteps):
    ...
    def save_data(self, obs_info):
        
        data_dir = self.config.IMG_DIR + "/data"
        rgb_dir = self.config.IMG_DIR + "/rgb"
        if not os.path.exists(data_dir):
            os.makedirs(data_dir, exist_ok=True)
        if not os.path.exists(rgb_dir):
            os.makedirs(rgb_dir, exist_ok=True)
            
        self.timestep = self.timestep + 1 if self.curr_node_step == self.last_nstep else 0
        self.last_nstep = self.curr_node_step
        
        obs_save = {k:v for k,v in obs_info.items() if k in ['rgb', 'depth', 'semantic', 'gps', 'compass', 'bbsgt', 'position', 'bbspred']}
        paths = save_obs(data_dir, self.rank, self.episode_no, obs_save, self.curr_node_step, self.timestep)
        
        img_str =rgb_dir+ f"/env_{self.rank:02d}_episode_{self.episode_no:06d}_glbstep_{self.curr_node_step:02d}_step_{self.timestep:05d}.png"
        
        if 'bbspred' in obs_save:
            img = cv2.resize(obs_save['rgb'], (256,256)) 
            visualizer = Visualizer(img,)
            vis_output = visualizer.draw_instance_predictions(
                        predictions=obs_save['bbspred']['instances'].to('cpu'))
            cv2.imwrite(img_str, vis_output.get_image().astype(np.uint8))
        else:
    ...
    def step(self, action, vis_info, *args, **kwargs):
        ...
        if self.video_option and done:
            ...
            generate_video(
                ...
                checkpoint_idx=0,
                metrics={"path_length": round(info["path_length"], 3)},
                tb_writer=None,
                fps=8,
            )
            ...
            metrics={
                        ...
                        "path_length": round(info["path_length"], 3),
