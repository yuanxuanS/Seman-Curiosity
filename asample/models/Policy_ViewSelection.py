...
from asample.models.utils import (
    angle_feature_with_ele, dir_angle_feature_with_ele, angle_feature_torch, length2mask)
import math
from asample.models.topograph.vlnbert_init import get_vlnbert_models
from src.policy_rl.utils.non_habitat_policy.rnn_state_encoder import LSTMStateEncoder
from asample.models.topograph.vilmodel_cmt import NextActionPrediction
from habitat_baselines.utils.common import CustomFixedCategorical

@baseline_registry.register_policy
class PolicyViewSelection(Policy):
    def __init__(
        self,
        ...
        self.global_sap_head = NextActionPrediction(self.net.vln_bert.config.hidden_size, 0.1)
        self.critic_head = CriticHead(self.net.vln_bert.config.hidden_size)
    @classmethod
    def from_config(
    ...
    def act(
        ...
        nav_logits = self.global_sap_head(gmap_embeds).squeeze(2)
        nav_logits.masked_fill_(gmap_masks.logical_not(), -float('inf'))
        ...
        distribution = CustomFixedCategorical(logits=nav_logits)
        if deterministic:
        ...
        aver_embeds = gmap_embeds.sum(1)
        values = self.critic_head(aver_embeds)
    ...
    def evaluate_actions(
        ...
        aver_embeds = gmap_embeds.sum(1)
        value = self.critic_head(aver_embeds)
...
class TopoGraphNet(Net):
    def __init__(
        ...
        print('\nInitalizing the TopoGraphNet model ...')
        ...
        assert model_config.DEPTH_ENCODER.cnn_type in [
            "ResnetDepthEncoder"
        ], "DEPTH_ENCODER.cnn_type must be ResnetDepthEncoder"
    ...
    def forward(self, mode=None, 
        ...
        if mode == 'waypoint':
            ...
            if in_train:
                # Waypoint augmentation, 训练时不是选极大抑制点，而是进行随机采样
        ...
        elif mode == 'navigation':      # TODO? 这是用于什么
