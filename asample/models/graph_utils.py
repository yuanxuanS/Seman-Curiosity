...
class GraphMap(object):
    def __init__(self, has_real_pos, loc_noise, merge_ghost, ghost_aug):
        ...
        self.loc_noise = loc_noise      # 定位节点的最小物理距离
    ...
    def _localize(self, qpos, kpos_dict, ignore_height=False):
        '''
        计算和图上所有点的最小距离， 如果小于阈值loc_noise, 定位导对应node的id，否则返回None，增加ghost node
        qpos: query pos, 位置
        kpos_dict: dict, 所有的id - pos
        '''
        min_dis = 10000
        min_vp = None
    ...
    def update_graph(self, prev_vp, step_id,
                           ...
                           cand_real_pos):
        '''
        prev_vp, cur_vp, cand_vp: node id, 整数
        step_id: 整数
        cur_pos: 位置
        '''
        # 1. connect prev_vp， 增加当前node，并和前一node连接
        ...
        for i, (cvp, cpos, cembeds) in enumerate(zip(cand_vp, cand_pos, cand_embeds)):
            ...
            # cand not overlap with node, create/update ghost
                ...
                if self.merge_ghost:
                    ...
                    # create ghost
                    if localized_gvp is None:       # 不和已有ghost node重复
                    ...
                    # update ghost
                    else:       # 和已有ghost重复，更新该ghost的信息
                        gvp = localized_gvp
                        self.ghost_pos[gvp].append(cpos)
                        self.ghost_mean_pos[gvp] = np.mean(self.ghost_pos[gvp], axis=0) # ghost的位置是所有的均值
                        self.ghost_embeds[gvp][0] = self.ghost_embeds[gvp][0] + cembeds
                        self.ghost_embeds[gvp][1] += 1
                        self.ghost_fronts[gvp].append(cur_vp)   # 记录该ghost的所有连接的node
                        if self.has_real_pos:
                            self.ghost_real_pos[gvp].append(cand_real_pos[i])
                else:   # 每个ghost单独建立，不融合
                    gvp = f'g{str(self.ghost_cnt)}'
                    self.ghost_cnt += 1
                    self.ghost_pos[gvp] = [cpos]
                    self.ghost_mean_pos[gvp] = cpos
        ...
        if self.ghost_aug != 0: # 添加噪声，主要是train时使用
    ...
    def get_pos_fts(self, cur_vp, cur_pos, cur_ori, gmap_vp_ids):
        '''返回node的pose encoding: 每个node相对现在位置的角度和距离：'''
        ...
        for vp in gmap_vp_ids:
            ...
            # for ghost
                ...
                front_dis, front_vp = self.front_to_ghost_dist(vp)  # ghost连接的visited node
